...
import { MapPin, Phone, Users, Bed, Search, ChevronDown, X, ArrowUpDown, UtensilsCrossed } from "lucide-react";
...
export default function Venues() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      case "name-az":
        list = [...list].sort((a, b) => (a.property_name || "").localeCompare(b.property_name || ""));
        break;
      case "name-za":
        list = [...list].sort((a, b) => (b.property_name || "").localeCompare(a.property_name || ""));
        break;
      case "cap-high":
        list = [...list].sort((a, b) => Number(b.max_banquet_capacity || 0) - Number(a.max_banquet_capacity || 0));
        break;
      case "cap-low":
        list = [...list].sort((a, b) => Number(a.max_banquet_capacity || 0) - Number(b.max_banquet_capacity || 0));
        break;
      case "rooms-high":
        list = [...list].sort((a, b) => Number(b.max_rooms || 0) - Number(a.max_rooms || 0));
        break;
    }
              ...
              >
                <div className="px-6 md:px-12 pb-3 flex flex-wrap items-center gap-2">
                  ...
                  {activeFilters.length > 1 && (
                    <button
                      onClick={clearAll}
                      className="flex items-center gap-1 px-3 py-1 text-white/40 font-manrope text-xs hover:text-white/70 transition-colors underline underline-offset-2"
                    >
                      Clear all
                    </button>
                  )}
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <span
                          className="font-cinzel text-[9px] uppercase tracking-[0.2em] font-bold px-2 py-1 rounded-sm bg-black/40 border border-white/10"
                      ...
                      <div className="mt-auto pt-4 space-y-2.5 font-manrope text-xs text-white/60">
                        {hasPhone && (
                          <div className="flex items-center gap-2.5">
                            ...
                            <a href={`tel:${venue.contact_number}`} className="font-mono hover:text-primary transition-colors truncate">
